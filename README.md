...
For headless or JSON-only setup, import a long-lived token generated by
`claude setup-token`:

```bash
curl -X POST http://localhost:20127/api/oauth/claude/import-token \
  -H 'Content-Type: application/json' \
  --data '{"accessToken":"sk-ant-oat...","name":"Claude CI"}'
```

The imported credential is stored as a non-refreshable `access_token`; generate
and import a new setup token when it expires or is revoked. The response returns
connection metadata only and never echoes the token.
